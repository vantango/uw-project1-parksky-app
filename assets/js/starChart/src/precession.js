...
function get_constellation(ra, dec) {  
    ...
    //var p = precessionMatrixVondrak(-1.25);
    var p11 = 0.9995359166195085, p12 = 0.027935620786950072, p13 = 0.012147118129294652, p21 = -0.027935619338023538, p22 = 0.9996097100023043, p23 = -0.00016982729487996506, p31 = -0.012147121461496999, p32 = -0.00016958878725908955, p33 = 0.9999262066171897;
    var x = Math.cos(dec)*Math.cos(ra);
    var y = Math.cos(dec)*Math.sin(ra);
    var z = Math.sin(dec);
    var x1 = p11*x + p12*y + p13*z;
    var y1 = p21*x + p22*y + p23*z;
    var z1 = p31*x + p32*y + p33*z;
    var rah = Math.atan2(y1,x1)*12.0/Math.PI;
    if (rah < 0) {rah +=24;}
    var decd = Math.asin(z1)*180.0/Math.PI;
    var conste = '';
    for (var i=0; i<constellation_boundary.length; i++) {
        if (decd > constellation_boundary[i][2] && 
            rah >= constellation_boundary[i][0] && 
           rah < constellation_boundary[i][1]) {
            conste = constellation_boundary[i][3];
            break;
        }
    }
    return conste;
