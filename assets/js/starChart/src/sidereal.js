...
var rotate1 = 0,
  rotate2 = 0; // rotationion angles (in rad) of charts 1 and 2
...
var starsLoc = [],
  brightestStarsLoc = [],
  conLabelLoc = [];
starsLoc[0] = brightStars();
starsLoc[1] = brightStars();
brightestStarsLoc[0] = brightestStars();
brightestStarsLoc[1] = brightestStars();
conLabelLoc[0] = constellationLabel();
conLabelLoc[1] = constellationLabel();
...
function changeLocationsAndTimes(form) {
  var place2Exists = form.place2in !== undefined ? true : false;
...
function starChartLoc(loc, drawOptions = null) {
  var d, lat, T0;
  if (loc == 1) {
    d = date1;
    lat = lat1;
  ...
  if (drawOptions === null) {
    // console.log(drawOptions.showPlanets);
...
function drawStarsPlanets(Canvas, objects, pDraw, LST, lat) {
  ...
  drawAzimuthLabels(Ctx, gpara, Ctx.strokeStyle);
  ...
  if (pDraw.showEcliptic) {
    ...
    pole = {
      ra: -0.5 * Math.PI,
      dec: pDraw.eclipticNorthPoleDec,
      linestyle: [10, 15],
      color: ecColor
    };
    drawCircle(Ctx, LST, cosLat, sinLat, pole, gpara);
...
function drawAzimuthLabels(Ctx, gpara, color = "black") {
...
function ra_dec_to_alt_az(raDec, LST, cosLat, sinLat) {
  ...
  var cosHA = Math.cos(HA),
    sinHA = Math.sin(HA);
  var cosDec = Math.cos(raDec.dec),
    sinDec = Math.sin(raDec.dec);
...
function drawCircle(Ctx, LST, cosLat, sinLat, pole, gpara) {
  ...
  if (Math.abs(pole.dec - gpara.halfPI) < 1e-5) {
    ...
    var sinHA = Math.sin(HA),
      cosHA = Math.cos(HA);
    var sinDec = Math.sin(pole.dec),
      cosDec = Math.cos(pole.dec);
  ...
  if (drawCir) {
    ...
    var sA = sinA * gpara.cosRotAng - cosA * gpara.sinRotAng;
    var cA = cosA * gpara.cosRotAng + sinA * gpara.sinRotAng;
    var x = gpara.xc + gpara.r * sA,
      y = gpara.yc + gpara.r * cA;
    Ctx.moveTo(x, y);
    for (var i = 1; i < n; i++) {
      var theta = i * dtheta;
...
function galacticNorthPole(T) {
  ...
  var ra0 = 3.366012906575397,
    dec0 = 0.4734787372451951;
...
function displayPopupSun(tip, para) {
  ...
  alt = alt.toFixed(2) + "&deg;";
  azi = azi.toFixed(2) + "&deg;";
...
function displayPopupStar(tip, para) {
  ...
  var rad_to_deg = 180 / Math.PI,
    rad_to_hr = 12 / Math.PI;
  var ra2000r = Math.atan2(y, x),
    dec2000r = Math.asin(z / distpc);
...
function setupDrawingParameters() {
  ...
  // planet order: Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune
  var pColor = ["orangered", "lemonchiffon", "chartreuse", "yellow", "deeppink", "chocolate", "violet", "cyan", "deepskyblue"];
  var pName = ["Sun", "Moon", "Mercury", "Venus", "Mars", "Jupiter", "Saturn", "Uranus", "Neptune"];
  var pSize = [1, 2, 1, 2, 2, 2, 2, 2, 2];
  var pCode = [9788, 9789, 9791, 9792, 9794, 9795, 9796, 9954, 9798];
  var offset = [
    { x: -20, y: 7 }, // sun -10,7
    { x: -17, y: 7 }, // moon -10,7
    { x: -20, y: 7 }, // mercury -5,7
    { x: -20, y: 0 }, // venus -7,0
    { x: -23, y: 7 }, // mars -7,2
    { x: -18, y: 7 }, // jupiter -10,7
    { x: -15, y: 7 }, // saturn -5,7
    { x: -23, y: 5 }, // uranus -10,3
    { x: -20, y: 5 } // neptune -8,5
  ];
